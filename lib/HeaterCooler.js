...
class HeaterCooler {
	constructor(device, entity, state, platform) {
		...
		if (!this.accessory) {
			...
			// Set last Target State to cache (fallback to AUTO=1 instead of COOL=2)
			...
			// Set last Target State to cache only if valid mode (1, 2, or 3)
...
this.HeaterCoolerService.getCharacteristic(Characteristic.Active)
  .onSet(async (value) => {
    this.log(`Active characteristic set to: ${value}`)

    if (value === 1) { // ON
      const modeToSet = [1, 2, 3].includes(this.accessory.context.lastTargetState) ? this.accessory.context.lastTargetState : 1
      this.log(`Restoring mode ${modeToSet} on Active ON`)

      this.state.mode = modeToSet
      this.accessory.context.lastTargetState = modeToSet

      let targetState = Characteristic.TargetHeaterCoolerState.AUTO
      if (modeToSet === 2) targetState = Characteristic.TargetHeaterCoolerState.COOL
      else if (modeToSet === 3) targetState = Characteristic.TargetHeaterCoolerState.HEAT

      await this.HeaterCoolerService.getCharacteristic(Characteristic.TargetHeaterCoolerState).setValue(targetState)
      await this.HeaterCoolerService.getCharacteristic(Characteristic.Active).updateValue(1)
  ...
  this.HeaterCoolerService.getCharacteristic(Characteristic.Active).updateValue(0)
		...
		// Validate lastTargetState; fallback to AUTO (1)
		const validLastTargetStates = [1, 2, 3]
		const lastState = validLastTargetStates.includes(this.accessory.context.lastTargetState)
			? this.accessory.context.lastTargetState
			: 1

		const targetMode = lastState === 1
	...
	updateState(state) {
		...
		// Update lastTargetState only if mode is valid (1, 2, or 3)
...
if (this.state.mode === 2) { // COOL
	this.HeaterCoolerService.getCharacteristic(Characteristic.TargetHeaterCoolerState)
		.updateValue(Characteristic.TargetHeaterCoolerState.COOL)

	const isCoolingNeeded = this.state.currentTemperature > this.state.targetTemperature
	const currentState = isCoolingNeeded
...
} else if (this.state.mode === 3) { // HEAT
	this.HeaterCoolerService.getCharacteristic(Characteristic.TargetHeaterCoolerState)
		.updateValue(Characteristic.TargetHeaterCoolerState.HEAT)

	const isHeatingNeeded = this.state.currentTemperature < this.state.targetTemperature
	const currentState = isHeatingNeeded
...
} else if (this.state.mode === 1) { // AUTO
	this.HeaterCoolerService.getCharacteristic(Characteristic.TargetHeaterCoolerState)
